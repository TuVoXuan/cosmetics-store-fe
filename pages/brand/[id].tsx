	...
	return (
		<div>
			<div className="space-y-14 mt-14 mb-14 md:mb-[112px] xl:mb-[144px]">
				<Breadcrumb className="hidden lg:block lg:mt-[93px]" items={["Trang chủ", "Thương hiệu"]} />
				<TitlePage
					className="capitalize mt-14 md:mt-16 xl:mt-12"
					subtitle={brandName}
					title={`sản phẩm của ${brandName}`}
				/>
						...
						<div className="flex flex-wrap items-center gap-4">
							{from && to && (
								<RemoveFilterButton
									...
									)}k - ${new Intl.NumberFormat("vn-VN").format(
										parseInt(to as string) / 1000
									)}k`}
								/>
							)}
							<p
								onClick={handleRemoveAllFilter}
								className="cursor-pointer text-red-accent text-paragraph-4"
							>
								Xóa tất cả
							</p>
						</div>
